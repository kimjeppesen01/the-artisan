...
trait FileHandler {
	 ...
	 */
	protected static function decodeFolderStructure($path, $type = array('php', 'css', 'js', 'png', 'jpg', 'jpeg', 'gif', 'svg', 'json', 'htm', 'html', 'asset.php')) {
	...
	public static function getFileURL($files, $folders = array(''), $debug = WP_DEBUG) {
		if($file = static::locateFile($files, $folders, $debug)) {
			$url = str_replace(trailingslashit(wp_normalize_path(WP_CONTENT_DIR)), trailingslashit(content_url()), $file);
