...
/**
 * Restructure product short description HTML for better scanability.
 * Wraps h2 sections and converts strong-label paragraphs to definition lists.
 */
function sa_restructure_product_description( $description ) {
	if ( empty( $description ) || ! is_product() ) {
		return $description;
	}

	// Convert <p><strong>Label:</strong> text</p> to <dl><dt>Label</dt><dd>text</dd></dl>
	$description = preg_replace_callback(
		'#<p>\s*<strong>([^<]+)</strong>\s*(.*?)</p>#s',
		function ( $m ) {
			$label = trim( rtrim( trim( $m[1] ), ':' ) );
			$text  = trim( $m[2] );
			return '<dl class="product-desc-feature"><dt>' . $label . '</dt><dd>' . $text . '</dd></dl>';
		},
		$description
	);

	// Wrap each h2 and its content (until next h2 or end) in a section
	$chunks = preg_split( '#(?=<h2)#i', $description );
	$out    = '';
	foreach ( $chunks as $chunk ) {
		...
		if ( preg_match( '#^<h2#i', $chunk ) ) {
			$out .= '<div class="product-desc-section">' . $chunk . '</div>';
		} else {
			$out .= $chunk;
		}
	}
...
add_filter( 'woocommerce_short_description', 'sa_restructure_product_description', 20 );